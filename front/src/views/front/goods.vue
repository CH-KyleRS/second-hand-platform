        ...
        <el-col :span="16">
          ...
          <!--  <div style="padding: 10px 0; color: #999;">
            ...
            <div style="margin-top: 20px; padding: 10px 0">
              ...
              <el-button style="margin-left: 10px; background-color: orangered; color: white" @click="buyNow">我想要
              </el-button>
            </div>
...
export default {
  ...
  methods: {
    ...
    praise() {
      ...
      this.entity = JSON.parse(JSON.stringify(this.goods))//在点赞功能中对其进行修改
    ...
    buyNow() {
      ...
      let cart = []
      cart.push({count: this.num, goods: this.goods, goodsId: this.goods.id})//会创建一个名为 cart 的数组，并向其中添加一项购物车数据。该购物车数据包含了购买数量 count、商品信息 goods 和商品 ID goodsId
      this.$store.commit("setCarts", cart)//调用 Vuex 的 mutation 方法，用于修改 Vuex 中的状态。
