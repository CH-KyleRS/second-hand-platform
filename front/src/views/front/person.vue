...
<!--                给 <el-upload> 组件添加了一个自定义的类名，用于自定义样式。
                通过 action 属性绑定了一个 uploadUrl 变量，指定了文件上传的接口地址。
                通过 show-file-list 属性设置为 false，隐藏上传成功后的文件列表。
                通过 on-success 属性绑定了一个 handleAvatarSuccess 方法，用于处理上传成功后的回调函数。
                通过 before-upload 属性绑定了一个 beforeAvatarUpload 方法，用于在上传前进行校验和处理
                通过条件渲染，当 imageUrl 存在时，显示一个图片元素，src 属性绑定了 imageUrl 变量，class 属性设置为 avatar，用于自定义样式。
                当 imageUrl 不存在时，显示一个图标元素，使用了 Element UI 的 el-icon-plus 图标，class 属性设置为 avatar-uploader-icon，用于自定义样式。-->
