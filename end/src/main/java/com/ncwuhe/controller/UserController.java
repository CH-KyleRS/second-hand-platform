...
public class UserController {
    ...
    public Result<User> login(@RequestBody User user){
        ...
        // 生成token  将用户的用户名作为 Token 的受众 (audience)，并使用用户的密码进行签名。这个 Token 将用于用户的身份验证和授权。
    ...
    public Result<?> recharge(@PathVariable BigDecimal money) {
        User user = getUser();
        user.setAccount(user.getAccount().add(money));//用户的账户金额与传入的money相加，并将结果设置回用户对象的账户属性。
    ...
    public Result<IPage<User>> findPage(@RequestParam(required = false, defaultValue = "") String name,
        ...
        //调用LambdaQueryWrapper对象的ne方法，添加一个不等于的条件，表示要查询的User对象的username属性不等于"admin"，即排除管理员用户。
